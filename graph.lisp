...
(defun a* (start goal cost-fun)
  (let ((open (list start))
    ...
    (labels ((score (node)
      ...
      (loop while open
            for current = (current)
            do (when (eq current goal)
                 (let ((path (list current)))
                   (loop for prev = (gethash current source)
                         while prev
                         do (setf current prev)
                            (push prev path))
                   (return path)))
               (setf open (delete current open))
               (dolist (port (ports current))
                 (dolist (con (connections port))
                   (let ((target (target-node current con)))
                     (when target
                       (let ((tentative-score (+ (score current) 1))
                             (cost (score target)))
                         (when (< tentative-score cost)
                           (setf (gethash target source) current)
